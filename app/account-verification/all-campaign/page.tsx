      ...
      <div className="grid grid-cols-3 border-b-8 border-[#F1F1F1] mb-6 [&>:last-child]:before:hidden">
        {["Approved", "Rejected", "Pending"].map((tab) => (
          <button
            key={tab}
            className={`tab-style
              ${activeTab === tab ? "text-black/80 after:opacity-100" : "text-black/40 after:opacity-0"}`}
          ...
          <tbody className='table-body-style [&_td]:text-center'>
            {filteredCampaigns.map((data, index) => (
              <tr key={index}>
                <td>{data.ID}</td>
                <td>
                  <Link className='hover:underline transition-all duration-300 hover:text-dark-blue' href={`/account-verification/all-campaign/${data.ID}`}>
                    {data.CampaignName}
                  </Link>
                </td>
